...
try:
    for i in range(1,10):
        if driver.find_element("xpath", f"/html[1]/body[1]/div[1]/div[3]/table[1]/tbody[1]/tr[1]/td[1]/div[3]/div[1]/div[2]/div[1]/div[1]/div[1]/div[1]/div[1]/form[1]/div[3]/div[4]/div[1]/div[1]/div[1]/div[1]/div[2]/div[1]/div[{str(i)}]/table[1]/tbody[1]/tr[1]/td[3]/div[1]/div[2]/a[1]/span[1]").get_attribute("innerText") == 'CR_2022_NSP_Ulteig':
...
for i in range(row_max_report-1):
    if statuses_report[i] == 'Initiate Application':
        if (IAnums_report[i] in IAnums_tracker) and (history_tracker[len(IAnums_tracker) - 1 - IAnums_tracker[::-1].index(IAnums_report[i])] == 'Rejected') and (dueDates_report[i] != dueDates_tracker[len(IAnums_tracker) - 1 - IAnums_tracker[::-1].index(IAnums_report[i])]):
            ...
            if developers_report[i] == 'All Energy Solar, Inc.' or developers_report[i] == 'Wolf River Electric' or developers_report[i] == 'Everlight Solar, LLC' or developers_report[i] == 'Everlight Solar' or developers_report[i] == 'Wolf River Electric - LTL LED LLC':
                wsTracker['H' + str(next_row_tracker)] = 'Joe Nogo'
            elif developers_report[i] == 'Blue Raven Solar' or developers_report[i] == 'BRS Field Ops, LLC' or developers_report[i] == 'BRS Field Ops, LLC.':
                wsTracker['H' + str(next_row_tracker)] = 'Ross K'
            else:
            ...
            wsTracker['Q' + str(next_row_tracker)] = 'JN'
            ...
            if types_report[i] == 'Yes':
                ...
                if flipFlop % 3 == 0:
                    driver.find_element("css selector", "[title='Meter Engineer Approver']").send_keys("Spencer Doriot")
                    print('Reassigned ' + str(IAnums_report[i]) + ' to Spencer Doriot (Larger than 40 kW)')
                elif flipFlop % 3 == 1:
                    driver.find_element("css selector", "[title='Meter Engineer Approver']").send_keys("Shaun Ly")
                    print('Reassigned ' + str(IAnums_report[i]) + ' to Shaun Ly (Larger than 40 kW)')
                else:
                    driver.find_element("css selector", "[title='Meter Engineer Approver']").send_keys("Chi Vang")
