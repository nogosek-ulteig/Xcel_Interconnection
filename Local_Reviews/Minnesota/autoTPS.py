...
for row in range(wsTracker.max_row):
    if row == 0 or row == 1:
        ...
        if (wsTracker['L' + str(row)].value == 'CA' or wsTracker['L' + str(row)].value == 'Approved') and (wsTracker['S' + str(row)].value == None):
...
for row in range(wsTracker.max_row):
    if row == 0 or row == 1:
        ...
        if (wsTracker['L' + str(row)].value == 'CA' or wsTracker['L' + str(row)].value == 'Approved') and (wsTracker['S' + str(row)].value == None and wsTracker['M' + str(row)].value != None):
            wsTracker['S' + str(row)].value = wsHelper['F' + str(counter)].value
            if wsHelper['F' + str(counter)].value == "Fail":
                wsTracker['T' + str(row)].value = date
            else:
                wsTracker['T' + str(row)].value = 'x'
            wsTracker['U' + str(row)].value = 'JN'
            counter += 1
